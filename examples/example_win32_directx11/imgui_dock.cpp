...
static void readLine(ImGuiContext* ctx, ImGuiSettingsHandler* handler, void* entry, const char* line_start)
{
    ...
    if( userdata )
    {
        ...
        else if(sscanf_s(line_start, "active=%d", &active) == 1)
        {
        ...
        else if(sscanf_s(line_start, "location=%[^\n^\r]", location) == 1)
        {
            strcpy_s( userdata->dock->location, location);
        }
        ...
        else if(sscanf_s(line_start, "parent=%d", &parent) == 1)
        {
...
bool ImGui::BeginDockspace()
{
	...
	sprintf_s( child_name , "##%s" , cur_dock_panel );
